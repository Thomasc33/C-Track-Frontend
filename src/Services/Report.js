...
export default {
    ...
    getJobCodeSummary: async (token, type) => {
        let res = await axios.get(`${BaseApiUrl}/reports/jobusage?type=${type}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-Version': require('../backendVersion.json').version } })
            .then(d => d.data)
            .catch(e => { console.warn(e.response.data); return { isErrored: true, error: e.response.data } })
        return res
    },
    getTsheetsData: async (user, date, token) => {
        let res = await axios.get(`${BaseApiUrl}/reports/tsheets?uid=${user}&date=${date}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-Version': require('../backendVersion.json').version } })
            .then(d => d.data)
            .catch(e => { console.warn(e.response.data); return { isErrored: true, error: e.response.data } })
        return res
    }
