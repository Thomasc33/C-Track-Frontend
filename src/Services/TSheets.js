...
export default {
    loginValidation: async (token, accessToken) => {
        let res = await axios.post(`${BaseApiUrl}/oauth/ts/login`, { accessToken }, { headers: { 'Authorization': `Bearer ${token}` } })
            .then(d => d.data)
            .catch(e => { console.warn(e.response.data); return { isErrored: true, error: e.response.data } })
        return res
    },
    getToken: async (token) => {
        let res = await axios.get(`${BaseApiUrl}/oauth/ts/verify`, { headers: { 'Authorization': `Bearer ${token}` } })
            .then(d => d.data)
